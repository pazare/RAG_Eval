...
- [ ] Configure 100-query slice and parameters in `config/default.yaml`.
...
- `config/default.yaml`: master settings for paths, models, retrieval depths, enhancement switches, RAGAs parameters, and random seeds (evaluation runs must use exactly 100 queries).
...
- `MilvusConfig.path = 'data/rag_wikipedia_mini.db'`, `collection_name = 'rag_mini'`
...
- Baseline and enhanced evaluations must run exactly 100 queries (assignment minimum).
- RAGAs analysis uses the full 100-sample evaluation set for comprehensive scoring, meeting assignment requirements.
...
- `results/modular_ragas_comparison.csv`
- `results/modular_ragas_delta.csv`
